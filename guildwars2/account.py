    ...
    async def account(self, ctx):
        ...
        if 'progression' in doc['permissions']:
            ...
            data.add_field(
                ...
                value=f"{user_ap} earned out of {possible_ap} possible",
                inline=False)
...
#### For the "BOSSES" command
    @commands.command(name='bosses', aliases=['kills'])
    @commands.cooldown(1, 10, BucketType.user)
    async def bosses(self, ctx):
        ...
        await ctx.send(
            f"{user.mention}, here are your raid bosses:", embed=embed)
                            
    ## Embeds boss information
    async def boss_embed(self, ctx, raids, results, account_name,
        ...
        if len(not_completed) == 0:
            ...
            if bosses:
                ...
                if len(bosses) > 1:
                    suffix = "es"
                bosses = f"{len(bosses)} boss{suffix}"
            if events:
                suffix = ""
                if len(events) > 1:
                    suffix = "s"
                events = f"{len(events)} event{suffix}"
...
#### For the "CATS" command
    ...
    async def cats(self, ctx):
        ...
        embed.set_footer(
            text=self.bot.user.name, icon_url=self.bot.user.avatar_url)
        await ctx.send(
            f"{user.mention}, here are the cats you don't have:", embed=embed)
        
#### For the "SEARCH" command
    @commands.command(name='search', usage='<item name>', aliases=['find'])
                ...
                if 'is_upgrade' in choice and choice['is_upgrade']:
                    ...
                    if k in char_names:
                        ...
                        if slotted_upg == 0:
                            ...
                            inf = f"/ {slotted_upg} "
                        output.append(f"{k.upper()} {' ' * (longest - len(k))} | {v[0]} {inf}")
                    else:
                        output.append(f"{k.upper()} {' ' * (longest - len(k))} | {v[0]}")
                else:
                    total += v[0]
                    total += v[1]
                    output.append(f"{k.upper()} {' ' * (longest - len(k))} | {v[0] + v[1]}")
        output.append(f"--------{'-' * (longest - 5)}------")
        output.append(f"TOTAL:{' ' * (longest - 2)}{total}")
        message = (f"{user.mention}, here are your search results:")
...
#### For the "LI" command
    @commands.command(name='li', aliases=['ld'])
    @commands.cooldown(1, 10, BucketType.user)
    async def li(self, ctx):
        ...
        embed.title = f"{total} Legendary {', '.join(trophy_names[:-1])} and {trophy_names[-1]} earned"
        embed.description = f"{total - crafted_total} on hand, {crafted_total} used in crafting."
        embed.set_author(name=doc['account_name'], icon_url=user.avatar_url)
        embed.set_thumbnail(
        ...
        embed.set_footer(
            text=self.bot.user.name, icon_url=self.bot.user.avatar_url)
        await ctx.send(
            f"{user.mention}, here are your Legendary Insights and Divinations:",
            embed=embed)

#### For the "KP" command
    @commands.command(name='kp', aliases=['killproof'])
    ...
    async def kp(self, ctx):
        ...
        await ctx.send(
            f"{user.mention}, here is your kill proof:", embed=embed)
