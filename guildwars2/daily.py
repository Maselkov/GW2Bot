...
#### For the "DAILY" group command
    @commands.group(case_insensitive=True, name='daily', aliases=['dailies', 'd'])
    async def daily(self, ctx):
        """Commands showing today's dailies"""
        if ctx.invoked_subcommand is None:
            await ctx.send_help(ctx.command)
            
  ### For the daily "ALL" command
  ...
  ### For the daily "PSNA" command
    @daily.command(name='psna')
    @commands.cooldown(1, 2, BucketType.user)
    async def daily_psna(self, ctx):
        """Shows Pact Supply Network Agent locations"""
    ...
    def get_fractals(self, fractals):
        ...
        for level in sorted(daily_recs, key=int):
            for k, v in fractals_data.items():
                if int(level) in v:
                    fractal_final.append(f"Recommended - {level} {k}")
