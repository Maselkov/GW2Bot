...
#### For the "CHATCODE" command
    @commands.command(name='chatcode', aliases=['cc'])
    async def chatcode(self, ctx):
        ...
        response = await self.user_input(
            ctx, "Type the item amount. (1-255)")
        if not response:
            return
        try:
        ...
        for k in upgrades:
            response = await self.user_input(
                ctx, f"Optionally, type the name of the {k} upgrade to apply to the "
                "item. Type `skip` to skip")
            if response.content.lower() == 'skip':
                break
            if not response:
        ...
        output = f"Here's your chatcode. No refunds. ```\n{chat_code}```"
...
#### For the "DULFY" command
    @commands.command(name='dulfy', usage='<search>', hidden=True)
    async def dulfy(self, ctx, *, search):
        ...
        url = base_url + f"?s={search}"
        message = await ctx.send(
            'Searching dulfy.net, this may take a while...')
...
#### For the "PRAISEJOKO" command
    @commands.command(name='praisejoko', hidden=True)
...
#### For the "WIKI" command
    @commands.command(name='wiki', usage='<search> <language>', aliases=['gw2wiki'])
