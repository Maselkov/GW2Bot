    ...
    def get_skill_fields(self, skill):
        ...
        for fact in sorted(
            ...
            if fact_type == 'BuffConversion':
                fields.append({
                    'field':
                    f"Gain {fact['target']} based on a Percentage of {fact['source']}",
                    'value':
                    "{fact['percent']}%"
                    })
            if fact_type == 'Damage':
                damage = calculate_damage(fact)
                value = f"{damage} ({round(fact['dmg_multiplier'] * fact['hit_count'],2)})"
    ...
    async def skill_embed(self, skill, ctx):
        ...
        if 'facts' in skill:
            ...
            for fact in facts:
                line = ""
                if fact.get('prefix'):
                    line += f"{find_closest_emoji(fact['prefix'])}{fact['prefix']}"
                line += f"{find_closest_emoji(fact['field'])}{fact['field']}"
                if fact.get('value'):
                    line += ": " + fact['value']
                for tup in replacement_attrs:
