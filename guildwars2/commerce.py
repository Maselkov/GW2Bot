...
#### For the "GEM" group command
    @commands.group(case_insensitive=True, name='gem', aliases=['gems'])
    async def gem(self, ctx):
        """Commands related to Gems"""
        if ctx.invoked_subcommand is None:
            await ctx.send_help(ctx.command)
            
  ### For the gem "PRICE" command
    ...
    async def gem_price(self, ctx, quantity: int = 400):
        ...
        data.add_field(
            name=f"{quantity} gems would cost you",
        ...
        data.add_field(
            name=f"{quantity} gems could buy you",
    ...
    async def get_coin_price(self, quantity=400):
        endpoint = f"commerce/exchange/gems?quantity={quantity}"
        results = await self.call_api(endpoint)
        return results['quantity']

  ### For the gem "TRACK" command
    @gem.command(name='track', usage='<gold amount>')
    async def gem_track(self, ctx, gold: int = 0):
        """Tracks the price of 400 gems.
        
        Receive a notification when the cost of 400 gems drops below a gold amount.

        Example: if you set the cost to 100, you will get a notification when the price of 400 gems drops below 100 gold."""
        user = ctx.author
        if not gold:
            ...
            if current:
                return await ctx.send(
                    "You'll currently be notified if the "
                    f"price of 400 gems drops below **{current // 10000}**")
            else:
  ...
  ### For the tp "CURRENT" command
    ...
    async def tp_current(self, ctx, buys_sells):
        ...
        if state == 'buys' or state == 'sells':
            ...
            return await ctx.send(
                f"{user.mention}, Please us either 'sells' or 'buys' as parameter")
        data = discord.Embed(
            description="Current " + state,
            colour=await self.get_embed_color(ctx))
        data.set_author(
            name=f"Transaction overview of {doc['account_name']}")
        data.set_thumbnail(
        ...
        endpoint_listing = f"commerce/listings?ids={str(item_id)}"
        # Call API once for all items
        try:
            listings = await self.call_api(endpoint_listing)
        except APIBadRequest:
            return await ctx.send(f"{user.mention} you don't have any ongoing "
                                  "transactions.")
        except APIError as e:
            return await self.error_handler(ctx, e)
        ...
        for result in results:
            ...
            undercuts = f"\nUndercuts: {undercuts}" if undercuts else ""
            if quantity == 1:
                total = ""
            else:
                total = "\nTotal: " + self.gold_to_coins(
                    ctx, quantity * price)
            data.add_field(
                name=item_name,
                value=f"{quantity} x {self.gold_to_coins(ctx, price)}{total}\n"
                      f"Max. offer: {self.gold_to_coins(ctx, max_price)} {undercuts}",
                inline=False)
...
#### For the tp "DELIVERY" command
    @tp.command(name='delivery')
    @commands.cooldown(1, 10, BucketType.user)
    async def tp_delivery(self, ctx):
        """Shows what's in your delivery box.
        ...
        data = discord.Embed(
            ...
            colour=await self.get_embed_color(ctx))
        data.set_author(
            name=f"Delivery overview of {doc['account_name']}")
        ...
        itemlist = []
        if coins == 0:
            gold = "Currently there are no coins for pickup."
        else:
...
#### For the tp "PRICE" command
    @tp.command(name='price', usage='<item name>')
    ...
    async def tp_price(self, ctx, *, item: str):
        ...
        try:
            ...
            return await ctx.send(f"{user.mention}, This item isn't on the TP."
                                  "")
        except APIError as e:
            return await self.error_handler(ctx, e)
        ...
        description = f"A level {level} {rarity.lower()} {itemtype.lower()}"
        if buyprice != 0:
