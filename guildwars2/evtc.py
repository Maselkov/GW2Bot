...
#### For the "EVTC" group command
    @commands.group(case_insensitive=True, name='evtc', aliases=['log'])
    async def evtc(self, ctx):
        """Processes an ArcDPS EVTC log
        
        Simply upload your file and in the "add a comment" field, type $evtc.
        Accepted formats are: .evtc, .zevtc, .zip
        
        Enabling compression in your ArcDPS will allow logs to mostly be within Discord's upload limit.
        """
  ...
  ### For the evtc "CHANNEL" command
    @commands.cooldown(1, 5, BucketType.guild)
    @commands.guild_only()
    @commands.has_permissions(manage_guild=True)
    @evtc.command(name='channel', usage='<channel name>')
    async def evtc_channel(self, ctx):
        """Automatically processes logs in a channel.
        ...
        await self.bot.database.set(ctx.channel, {'evtc.enabled': enabled},
                                    self)
        if enabled:
            msg = ("Automatic EVTC processing enabled. Simply upload files "
                   "you want processed into this channel.\nAccepted "
                   "formats: `.evtc`, `.zevtc`, `.zip` ")
            if not ctx.channel.permissions_for(ctx.me).embed_links:
                await ctx.send("Need permission to embed links.")
        else:
            msg = ("Automatic EVTC processing disabled.")
        await ctx.send(msg)
