...
from .utils.chat import embed_list_lines, en_space, magic_space, zero_width_space
...
class EvtcAutouploadDestinationsSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for choice in choices:
            await self.cog.db.evtc.destinations.delete_one({"_id": choice["_id"]})
        await interaction.response.edit_message(
...
class EvtcMixin:
        ...
        if not token:
            try:
                async with self.session.get(TOKEN_URL) as r:
                    ...
                    await self.bot.database.set(user, {"dpsreport_token": token}, self)
                    return token
            except Exception:
                return None
    ...
    async def upload_log(self, file, user, *, anonymous=False):
        ...
        async with self.session.post(UPLOAD_URL, data=data, params=params) as r:
            resp = await r.json()
            error = resp["error"]
            if error:
    ...
    async def get_encounter_data(self, encounter_id):
        async with self.session.get(JSON_URL, params={"id": encounter_id}) as r:
            return await r.json()
    ...
    async def upload_embed(self, destination, data, permalink):
        ...
        for target in targets:
            group_dps += sum(p["dpsTargets"][target][0]["dps"] for p in data["players"])
        ...
        for player in players:
            ...
            prof = self.get_emoji(destination, player["profession"], force_emoji=True)
            ...
            for buff in buffs:
                for buff_uptime in player["buffUptimes"]:
                    if buff["id"] == buff_uptime["id"]:
                        uptime = str(
                            round(buff_uptime["buffData"][0]["uptime"], 1)
                        ).rjust(5)
                        break
                else:
                    uptime = "0"
        ...
        doc = {
            ...
            "date": date,
            "players": sorted([player["account"] for player in data["players"]]),
            "permalink": permalink,
        ...
        embed.set_footer(text="Recorded at", icon_url=self.bot.user.display_avatar.url)
        if boss:
    ...
    async def evtc(
        ...
        await self.process_evtc(
            [file], interaction.user, interaction.followup, anonymous=anonymous
        )
    ...
    async def evtc_channel(
        ...
        await self.bot.database.set(
            channel, {"evtc.enabled": enabled, "evtc.autodelete": autodelete}, self
        )
        if enabled:
            msg = (
                "Automatic EVTC processing enabled. Simply upload the file "
            ...
            if not channel.permissions_for(interaction.guild.me).embed_links:
                msg += (
                    "I won't be able to process logs without Embed " "Links permission."
                )
        else:
    ...
    async def get_user_evtc_api_key(self, user: discord.User) -> Union[str, None]:
    ...
    async def evtc_api_key(self, interaction: discord.Interaction, operation: str):
        ...
        if operation == "delete":
            ...
            await self.db.evtc.api_keys.delete_one({"_id": interaction.user.id})
            return await interaction.followup.send(
        ...
        if operation == "generate":
            ...
            await self.db.evtc.api_keys.insert_one(
                {"user": interaction.user.id, "token": key}
            )
    ...
    async def get_evtc_notification_channel(self, id, user):
        await self.db.evtc.channels.find_one({"channel_id": id, "user": user.id})

    @autopost_group.command(name="add_destination")
    @app_commands.checks.has_permissions(embed_links=True, use_external_emojis=True)
    @app_commands.checks.bot_has_permissions(embed_links=True, use_external_emojis=True)
    async def evtc_autoupload_add(self, interaction: discord.Interaction):
        ...
        doc = (
            await self.db.evtc.destinations.find_one(
        ...
        await self.db.evtc.destinations.insert_one(
            {
                ...
                "guild_ids": view.selected_guilds,
                "guild_tags": [
                    guild["tag"]
                    for guild in guilds
    ...
    async def evtc_autoupload_remove(self, interaction: discord.Interaction):
        ...
        channels = [self.bot.get_channel(dest["channel_id"]) for dest in destinations]
        if not channels:
            return await interaction.followup.send(
                "You don't have any autopost destinations yet."
            )
        view = discord.ui.View()
        view.add_item(EvtcAutouploadDestinationsSelect(self, channels, destinations))
        await interaction.followup.send("** **", view=view)

    async def process_evtc(
        self, files: list[discord.Attachment], user, destination, anonymous=False
    ):
        embeds = []
        for attachment in files:
            if attachment.filename.endswith(ALLOWED_FORMATS):
                try:
                    resp = await self.upload_log(attachment, user, anonymous=anonymous)
                    data = await self.get_encounter_data(resp["id"])
                    embeds.append(
                        await self.upload_embed(destination, data, resp["permalink"])
                    )
                except Exception as e:
                    self.log.exception("Exception processing EVTC log ", exc_info=e)
            ...
            try:
                ...
                for destination in destinations:
                    try:
                        ...
                        if (
                            destination["guild_ids"]
                            and recorded_by
                            and recorded_player_guild
                        ):
                            if destination["guild_ids"]:
                                if (
                                    recorded_player_guild
                                    not in destination["guild_ids"]
                                ):
                                    continue
                        if not channel:
                            continue
                        ...
                        if guild := channel.guild:
                            if not guild.chunked:
                                await guild.chunk()
                            members = [channel.guild.me, guild.get_member(user.id)]
                ...
                self.log.exception("Exception during evtc notificaitons", exc_info=e)
            finally:
    ...
    async def on_message(self, message: discord.Message):
        ...
        await self.process_evtc(message.attachments, message.author, message.channel)
        if autodelete:
            try:
                if message.channel.permissions_for(message.channel.me).manage_messages:
