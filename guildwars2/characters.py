...
#### For the "CHARACTER" group command
    @commands.group(case_insensitive=True, name='character')
    async def character(self, ctx):
        """Character related commands"""
        if ctx.invoked_subcommand is None:
            await ctx.send_help(ctx.command)

  ### For the character "BIRTHDAYS" command
    @character.command(name='birthdays')
    async def character_birthdays(self, ctx):
        """Lists character birthdays.
        
        This sists days until a character's next birthday for each character.
        ...
        msg = f"{user.mention}, here are your upcoming birthdays:"
        embed = discord.Embed(
  ...
  ### For the character "FASHION" command
    @character.command(name='fashion', usage='<character name>')
    @commands.cooldown(1, 10, BucketType.user)
    async def character_fashion(self, ctx, *, character: str):
        """Lists a character's fashion.
        ...
        embed.set_footer(
            text=f"A level {level} {profession.name.lower()} ",
  ...
  ### For the character "GEAR" command. This is for PvE
    @character.command(name='gear', usage='<character name>')
    @commands.cooldown(1, 10, BucketType.user)
    async def character_gear(self, ctx, *, character: str):
        """Displays PvE gear, stats, and build of a character.
    ...
    ### For the character "GEAR" command. These are for hidden aliases
    @character.command(name='pvegear', usage='<character name>', aliases=['pvebuild', 'attributes', 'build'], hidden=True)
    @commands.cooldown(1, 10, BucketType.user)
    async def character_pvegear(self, ctx, *, character: str):
        """Displays PvE gear, stats, and build of a character.
  ...
  ### For the character "PVPGEAR" command. This is for PvP
    @character.command(name='pvpgear', usage='<character name>', aliases=['pvpbuild'])
    @commands.cooldown(1, 10, BucketType.user)
    async def character_pvpgear(self, ctx, *, character: str):
        """Displays PvP gear, stats and build of a character.
  ...
  ### For the character "WVWGEAR" command. This is for WvW
    @character.command(name='wvwgear', usage='<character name>', aliases=['wvwbuild'])
    @commands.cooldown(1, 10, BucketType.user)
    async def character_wvwgear(self, ctx, *, character: str):
        """Displays WvW gear, stats, and build of a character.
        You must be the owner of the character.

        Required permission: characters"""
        await self.display_gear(ctx, character, 'wvw')

    ## For the character "GEAR|PVPGEAR|WVWGEAR" commands
  ...
  ### For the character "INFO" command
    @character.command(name='info', usage='<character name>')
    ...
    async def character_info(self, ctx, *, character: str):
        ...
        if guild is not None:
            endpoint = f"guild/{results['guild']}"
            try:
            ...
            gname = guild['name']
            gtag = guild['tag']
            data.add_field(name="Guild", value=f"[{gtag}] {gname}")
        ...
        data.set_footer(text=f"A {gender.lower()} {race} {profession.name.lower()}")
        try:
            await ctx.send(embed=data)
        except discord.Forbidden:
            await ctx.send("Need permission to embed links.")

  ### For the character "LIST" command
    @character.command(name='list', usage='<name|profession|created|age>')
    @commands.cooldown(1, 5, BucketType.user)
    async def character_list(self, ctx, sort='name'):
        """Lists all your characters.
        
        List sorting options:
        name, profession, date of creation, total time played
        Defaults to name.
        
        Required permissions: characters, builds
        """
        ...
        embed.set_footer(text="You can use age|created|profession to "
                         f"display more information! E.g. {ctx.prefix}character list age")
        await ctx.send(f"{user.mention}", embed=embed)

  ### For the character "TOGGLEPUBLIC" command. Currently out of date
    @character.command(name='togglepublic', usage='<character|all>')
    @commands.cooldown(1, 1, BucketType.user)
    async def character_togglepublic(self, ctx, *, character_or_all: str):
        """Toggle character status.
        
        Toggling this with a given character makes the character's information public.
        ...
        await ctx.send("Character status successfully changed. Anyone can "
                       "check a public character's gear and build - the rest is "
                       "still private.\nTo make a character private "
                       "again, type the same command.")
        if character == 'All':
            await user.send("\n".join(output))
			
#### For the "SAB" group command
    @commands.group(case_insensitive=True, name='sab')
    async def sab(self, ctx):
        """Super Adventure Box commands"""
        if ctx.invoked_subcommand is None:
            await ctx.send_help(ctx.command)
			
  ### For the sab "UNLOCKS" command
    @sab.command(name='unlocks', usage='<character name>')
    @commands.cooldown(1, 10, BucketType.user)
    async def sab_unlocks(self, ctx, *, character):
        """Displays missing SAB unlocks for a character.
        You must be the owner of the character.
        ...
        endpoint = f"characters/{character}/sab"
        try:
  ...
  ### For the sab "ZONES" command
    @sab.command(name='zones', usage='<character name>', aliases=['worlds'])
    @commands.cooldown(1, 10, BucketType.user)
    async def sab_zones(self, ctx, *, character):
        """Displays missing SAB zones for a character.
                ...
                for mode in modes:
                    for zone in range(1, number_of_zones + 1):
                        ...
                        if zone_dict not in zones:
                            missing.append(f"W{world}Z{zone} {mode.title()} mode")
            return missing
    ...
    def get_crafting(self, character):
        ...
        for crafting in character['crafting']:
            ...
            craft_list.append(f"Level {rating} {discipline}")
        return craft_list
